...
- Пакет `EncySoftware.DmcMcp`, команда `dmc-mcp`, имя сервера `dmc`, namespace `DmcMcp`.
...
<Project Sdk="Microsoft.NET.Sdk">
  <PropertyGroup>
    ...
    <PackageId>EncySoftware.DmcMcp</PackageId>
    <Version>0.1.0</Version>
    <Authors>EncySoftware</Authors>
...
printf '%s\n' \
 ...
 '{"jsonrpc":"2.0","id":2,"method":"tools/list"}' > /tmp/smoke-in.json
# stdin держим открытым ещё ~3 с: при мгновенном EOF транспорт SDK считает клиента ушедшим
# и ответы не пишет (проверено 2026-09-16 — stdout был пуст при exit 0). `ping` вместо `sleep`.
( cat /tmp/smoke-in.json; ping -n 4 127.0.0.1 > /dev/null ) \
 | dotnet run --project src --no-build -c Debug 2>/dev/null \
 | grep -oE '"name":"(publish_post|update_post|submit_post|check_post_status)"' | sort -u
```
...
```bash
dotnet tool install -g EncySoftware.DmcMcp --add-source <папка с .nupkg>
dmc-mcp setup
```
...
Expected: `EncySoftware.DmcMcp.0.1.0.nupkg` (папка `pkg/` в `.gitignore`).
...
(Юрий) на своей машине: `dotnet tool install -g EncySoftware.DmcMcp --add-source pkg`,
